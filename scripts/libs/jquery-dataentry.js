  ...
  _.extend(DataEntry.prototype, {
    ...
    validate: function (fields, options) {
      ...
      for (var x in this.schema) {
        ...
        chain.push(this.validateField(x, options));
      }
     ...
     * @returns {boolean}
     */
    validateSync: function (fields, options) {
      if (fields && _.isFunction(fields)) fields = fields.call(this);
      if (fields && !_.isArray(fields)) throw 'invalid parameter: fields must be an array of strings';
      //if this context has no element, throw exception
      if (!this.$el || !this.$el.length) throw 'missing $el property, or missing element';
      ...
      for (x in this.schema) {
        if (fields && !_.contains(fields, x)) continue;
        if (valid)
          valid = this.validateFieldSync(x, options);
        else
          this.validateFieldSync(x, options);
      }
      return valid;
    },
    ...
    validateField: function (fieldName, options) {
      ...
      if (!field.length || (options.onlyIfTouched && !field.hasClass("ui-touched"))) return;
    ...
    getSpecialEvents: function () {
      ...
      var changeCallback = function (e, forced, fromKnockout) {
        //knockout fires the change event programmatically, which is against HTML standards
        if (fromKnockout) return true;
        //add a class to the element
        this.formatter.marker.markFieldTouched($(e.currentTarget));
        this.validationActive = true;
        //trigger validation
